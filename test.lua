...
for _, v in next, workspace:WaitForChild("Buildings"):GetChildren() do
    tab2:CreateButton(v.Name, function()
        Root:PivotTo(v:FindFirstChildWhichIsA("Model").CollisionBox.CFrame + Vector3.new(0, 15, 0))
    end)
end
...
tab3:CreateButton("Забрать награды", function()
...
tab3:CreateButton("Собрать все монеты", function()
    for _, v in next, workspace.CurrencyDrops:GetChildren() do
